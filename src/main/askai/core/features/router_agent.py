...
class RouterAgent(metaclass=Singleton):
    ...
    def _assert_actions(actions, *attrs):
        ...
        :param actions: The action namespaces to assert.
        """
        check_argument(actions is not None and len(actions) > 0, "Invalid Actions (None or Empty)")
        check_argument(all(isinstance(act, SimpleNamespace) for act in actions), "Invalid action format (JSON)")
        check_argument(all(hasattr(act, attr) for attr in attrs for act in actions), "Invalid action arguments")
