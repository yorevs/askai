...
def assert_accuracy(question: str, ai_response: str) -> Optional[str]:
    ...
    if not ai_response:
        reason = (
            f"The AI provided an EMPTY answer\n"
    ...
    if response and (output := response.content):
        if mat := RagResponse.matches(output):
            ...
            if RagResponse.of_value(status.strip()).is_bad:
                shared.context.push("CONTEXT", (
                    f"The AI provided a BAD answer:\n'{output}'\n\n"
                    "Please try again improving your response. Address the problems above.\n(Reminder "
                    "to respond using at least one tool, stick to the tools syntax and avoid responding directly).\n"
                ))
...
@lru_cache
def stt(question: str, context: str | None) -> str:
...
def replace_x_refs(question: str, context: str) -> Optional[str]:
    ...
    AskAiEvents.ASKAI_BUS.events.reply.emit(message=msg.x_reference(context), verbosity='debug')
